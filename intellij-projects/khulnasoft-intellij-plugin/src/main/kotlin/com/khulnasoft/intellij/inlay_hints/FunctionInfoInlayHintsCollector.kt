...
package com.khulnasoft.intellij.inlay_hints

import com.khulnasoft.intellij.chat_server_client.ChatServerClient
import com.khulnasoft.intellij.chat_window.KHULNASOFT_CHAT_ID
import com.khulnasoft.intellij.chat_window.ChatViewerWindowService
import com.khulnasoft.intellij.isInjectedText
...
class FunctionInfoInlayHintsCollector(private val file: PsiFile, private val editor: Editor) :
  ...
  override fun collect(element: PsiElement, editor: Editor, sink: InlayHintsSink): Boolean {
    ...
    var functionInfos: List<KhulnasoftCommon.FunctionInfo>? = null
    runBlocking {
      try {
    ...
    for (functionInfo in functionInfos!!) {
      ...
      val refactorPresentation =
          factory.referenceOnHover(factory.smallText("Refactor")) { _, _ ->
            println("[Khulnasoft] Refactor function `${functionInfo.nodeName}`")

            // Open the Khulnasoft chat tool window.
            ToolWindowManager.getInstance(file.project).getToolWindow(KHULNASOFT_CHAT_ID)?.show()
      ...
      val explainPresentation =
          factory.referenceOnHover(factory.smallText("Explain")) { _, _ ->
            println("[Khulnasoft] Explain function `${functionInfo.nodeName}`")
      ...
      val docstringPresentation =
          factory.referenceOnHover(factory.smallText("Docstring")) { _, _ ->
            println("[Khulnasoft] Generate docstring for function `${functionInfo.nodeName}`")
