...
package com.khulnasoft.intellij.language_server

import com.khulnasoft.intellij.KhulnasoftWorkspaceTracker
import com.khulnasoft.intellij.EditorManager
...
import ksoft.khulnasoft_common_pb.KhulnasoftCommon.UserStatus
import ksoft.language_server_pb.LanguageServer
import ksoft.seat_management_pb.SeatManagement
import ksoft.seat_management_pb.SeatManagementServiceGrpcKt
...
const val KHULNASOFT_DEV_MODE_ENV_VAR = "KHULNASOFT_DEV_MODE"
val MANAGER_DIR = Path("khulnasoft", "manager")
...
const val AcceptCompletionActionId = "com.khulnasoft.intellij.AcceptCompletionAction"
const val DismissCompletionActionId = "com.khulnasoft.intellij.DismissCompletionAction"
...
class LanguageServerLauncherActivity : StartupActivity.Background {
  ...
  override fun runActivity(project: Project) {
    ...
    CoroutineScope(Dispatchers.IO).launch {
      val khulnasoftAuthService = service<KhulnasoftAuthService>()

      CoroutineScope(Dispatchers.IO).launch { khulnasoftAuthService.maybeShowLoginPrompt(project) }
      ...
      val pluginPath = Path(PathManager.getPluginsPath()).resolve("khulnasoft")
      ...
      if (!Files.exists(languageServerPath, LinkOption.NOFOLLOW_LINKS)) {
        khulnasoftStatusService.updateState(
            LanguageServer.State.newBuilder()
                .setState(LanguageServer.KhulnasoftState.KHULNASOFT_STATE_PROCESSING)
                .setMessage("Downloading language server")
                .build())
        try {
          var extensionBaseUrl = "https://github.com/Exafunction/khulnasoft/releases/download"
      ...
      val apiServerUrl = khulnasoftStatusService.getApiServerUrl()
      ...
      if (System.getenv(KHULNASOFT_DEV_MODE_ENV_VAR) != null) {
        cmdArgs.add("--dev_mode")
      }
      if (khulnasoftStatusService.hasEnterprisePlugin()) {
        cmdArgs.add("--portal_url")
        cmdArgs.add(khulnasoftStatusService.getWebsiteUrl())
        cmdArgs.add("--enterprise_mode")
      }

      if (khulnasoftAuthService.isLoggedIn()) {
        val channel = ManagedChannelBuilder.forAddress("server.khulnasoft.com", 443).build()
            ...
            (userStatus != null) &&
                (userStatus.teamId.isNotEmpty() &&
                    userStatus.teamStatus.equals(
                        KhulnasoftCommon.UserTeamStatus.USER_TEAM_STATUS_APPROVED))
        if (isTeams) {
          cmdArgs.add("--teams_mode")
          service<KhulnasoftAuthService>().isTeams = true
        }
      }
      // Create database directory in ~/.khulnasoft/database
      val homeDirectory = System.getProperty("user.home")
      val databaseDir = Path(homeDirectory, ".khulnasoft", "database", MODEL_MD5)
        ...
        withTimeout(MANAGER_DIR_PORT_TIMEOUT_SEC * 1000) {
          while (true) {
            ...
            if (portFile != null) {
              ...
              service<KhulnasoftAuthService>().setAuthPort(port)
              project.service<ChatViewerWindowService>().maybeShowChatWindow()
      ...
      if (startedLanguageServer) {
        service<KhulnasoftWorkspaceTracker>().updateRootsForProject(project)
      }
    }
  }
}
