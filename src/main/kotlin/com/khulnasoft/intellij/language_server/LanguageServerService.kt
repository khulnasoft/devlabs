...
import com.khulnasoft.intellij.getLanguage
...
import ksoft.language_server_pb.LanguageServerServiceGrpcKt
...
class LanguageServerService : Disposable {
  ...
  fun getMetadata(): KhulnasoftCommon.Metadata {
    ...
    if (apiKey.isEmpty()) {
      khulnasoftStatusService.updateState(
          LanguageServer.State.newBuilder()
              ...
              .setMessage("Not logged into Khulnasoft")
              .build())
      throw Exception("API key is empty")
    }

    val version = khulnasoftStatusService.getVersion()
    return KhulnasoftCommon.Metadata.newBuilder()
  ...
  suspend fun getCompletions(
      editor: Editor,
      file: PsiFile,
      editorOptions: KhulnasoftCommon.EditorOptions,
      offset: Int
    ...
    return try {
      ...
      khulnasoftStatusService.updateState(response.state)
      ...
      if (e.status.code == Status.Code.ABORTED) {
        ...
        service<KhulnasoftStatusService>().showAbortErrorNotification(e.status, editor)
        return emptyList()
      }
      ...
      service<KhulnasoftStatusService>()
          .updateState(
              LanguageServer.State.newBuilder()
                  .setState(LanguageServer.KhulnasoftState.KHULNASOFT_STATE_ERROR)
                  .setMessage("Disconnected from Khulnasoft Server")
                  .build())
      emptyList()
    }
  }
  ...
  suspend fun recordEvent(event: KhulnasoftCommon.Event) {
    if (!languageServerStarted) return
    val request =
  ...
  suspend fun getFunctions(editor: Editor, file: PsiFile): List<KhulnasoftCommon.FunctionInfo> {
    if (!languageServerReady()) {
  ...
  suspend fun getUserStatus(): KhulnasoftCommon.UserStatus {
    if (!languageServerReady()) {
      return KhulnasoftCommon.UserStatus.newBuilder().build()
    }
    val request =
    ...
    return try {
      ...
      KhulnasoftCommon.UserStatus.newBuilder().build()
    }
  }
