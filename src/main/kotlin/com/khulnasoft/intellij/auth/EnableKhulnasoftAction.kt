...
class EnableKhulnasoftAction(private val languageId: String = "*", private val language: String = "") :
    AnAction(), PopupAction, DumbAware {
  override fun actionPerformed(e: AnActionEvent) {
    val khulnasoftStatusService = service<KhulnasoftStatusService>()
    if (languageId != "*" && !khulnasoftStatusService.isKhulnasoftEnabled("*")) {
      // You cannot enable Khulnasoft for a specific language if it is disabled for all languages
      return
    }
  ...
  override fun update(e: AnActionEvent) {
    ...
    e.presentation.text =
        if (languageId == "*") {
          "Enable Khulnasoft Globally"
        } else {
          "Enable Khulnasoft for $language$experimental"
        }
  }
}
