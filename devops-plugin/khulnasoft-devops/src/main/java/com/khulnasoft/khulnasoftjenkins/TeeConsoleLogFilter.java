...
import com.khulnasoft.khulnasoftjenkins.utils.PlainTextConsoleUtils;
...
import static com.khulnasoft.khulnasoftjenkins.Constants.CONSOLE_TEXT_SINGLE_LINE_MAX_LENGTH;
import static com.khulnasoft.khulnasoftjenkins.Constants.LOG_TIME_FORMAT;
import static com.khulnasoft.khulnasoftjenkins.model.EventType.CONSOLE_LOG;
import static java.nio.charset.StandardCharsets.UTF_8;


/**
 * work like unix tee, one end is khulnasoft http output, the other is console out
...
public class TeeConsoleLogFilter extends ConsoleLogFilter implements Serializable {
    ...
    public OutputStream decorateLogger(Run build, OutputStream output) throws IOException, InterruptedException {
        ...
        return teeOutput(output, logSource, useLineNumber, KhulnasoftJenkinsInstallation.get().getMaxEventsBatchSize());
    ...
    public OutputStream decorateLogger(Computer computer, OutputStream logger) throws IOException, InterruptedException {
        ...
        long cacheSize = Math.min(KhulnasoftJenkinsInstallation.get().getMaxEventsBatchSize(), Constants.SLAVE_LOG_BUFFER_SIZE);
    ...
    private OutputStream teeOutput(OutputStream output, String source, boolean useLineNumber, long cacheSize) {
        if (KhulnasoftJenkinsInstallation.get().isEventDisabled(CONSOLE_LOG)) {
            return output;
        }
        if (KhulnasoftJenkinsInstallation.get().isJobIgnored(source)) {
            return output;
        }
    ...
    public static class TeeOutputStream extends FilterOutputStream {
        ...
        public TeeOutputStream(OutputStream out, String sourceName) {
            ...
            LOG.log(Level.FINE, "created khulnasoft output tee for " + sourceName);
        }
        ...
        private void flushLog() {
            ...
            try {
                ...
                KhulnasoftLogService.getInstance().send(logs, CONSOLE_LOG, sourceName);
