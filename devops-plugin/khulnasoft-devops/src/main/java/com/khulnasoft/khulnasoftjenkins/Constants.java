...
public class Constants {
    ...
    public static final int MIN_BUFFER_SIZE = Integer.getInteger("khulnasoftins.buffer", 4096);
    public static final int JDK_FINE_LOG_BATCH = Integer.getInteger("khulnasoftins.debugLogBatchSize", 128);
    ...
    // if it is increased, please also increase the TRUNCATE config in khulnasoft props.conf
    // ref: http://docs.khulnasoft.com/Documentation/Khulnasoft/7.2.1/Admin/Propsconf
    public static final int CONSOLE_TEXT_SINGLE_LINE_MAX_LENGTH = Integer.getInteger("khulnasoftins.lineTruncate", 100000);
    ...
    public static final int MAX_JUNIT_STDIO_SIZE = Integer.parseInt(System.getProperty("khulnasoftins.junitStdioLimit", "2097152"));
    public static boolean DECODE_PIPELINE_CONSOLE = Boolean.parseBoolean(System.getProperty("khulnasoftins.decodePipelineConsole", "true"));
    public static final boolean ENABLE_POST_LOGGER = Boolean.parseBoolean(System.getProperty("khulnasoftins.auditPostRequest", "true"));
    public static final Pattern JOB_CONSOLE_FILTER_WHITELIST_PATTERN;

    static {
        Pattern filterPattern = null;
        String filterStr = System.getProperty("khulnasoftins.allowConsoleLogPattern", "");
        if (StringUtils.isNotBlank(filterStr)) {
            try {
                filterPattern = Pattern.compile(filterStr);

            } catch (PatternSyntaxException ex) {
                Logger.getLogger("com.khulnasoft.khulnasoftjenkins.KhulnasoftJenkinsInstallation").log(Level.SEVERE,
