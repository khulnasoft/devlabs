...
import static com.khulnasoft.khulnasoftjenkins.model.EventType.JENKINS_CONFIG;
...
public class WebPostAccessLogger implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        LOG.info("khulnasoft-filter loaded");
    }
    ...
    private void loggerUserAction(ServletRequest servletRequest) {
        if (KhulnasoftJenkinsInstallation.get().isEventDisabled(JENKINS_CONFIG)) {
            return;
        }
        ...
        KhulnasoftLogService.getInstance().send(auditInfo, JENKINS_CONFIG, "web_access");
    }
